...
public class OverloadPotionTask extends Task {
	...
	/*	int timer = player.getOverloadTimer();
		...
		player.setOverloadTimer(timer - 1);
		if(player.getOverloadTimer() == 20) 
			player.getPacketSender().sendMessage("@red@Your Overload's effect is about to run out.");*/
	//	if(player.getOverloadTimer() <= 0 /*|| player.getLocation() == Location.DUEL_ARENA || player.getLocation() == Location.DUNGEONEERING || player.getLocation() == Location.WILDERNESS && player.getRights() != PlayerRights.DEVELOPER*/) {
