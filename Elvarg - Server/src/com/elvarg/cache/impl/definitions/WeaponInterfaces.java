...
public final class WeaponInterfaces {
	...
	public enum WeaponInterface {
		STAFF(328, 355, 5,
				new FightType[] { FightType.STAFF_BASH, FightType.STAFF_POUND, FightType.STAFF_FOCUS }), WARHAMMER(425,
						...
						7474, 7486), MAUL(425, 428, 5,
								new FightType[] { FightType.MAUL_POUND, FightType.MAUL_PUMMEL, FightType.MAUL_BLOCK },
								7474, 7486), GRANITE_MAUL(425, 428, 6,
										new FightType[] { FightType.GRANITE_MAUL_POUND, FightType.GRANITE_MAUL_PUMMEL,
												FightType.GRANITE_MAUL_BLOCK },
										7474,
										7486), SCYTHE(776, 779, 4, new FightType[] { FightType.SCYTHE_REAP,
												FightType.SCYTHE_CHOP, FightType.SCYTHE_JAB,
												FightType.SCYTHE_BLOCK }), BATTLEAXE(1698, 1701,
														5,
														new FightType[] { FightType.BATTLEAXE_CHOP,
																FightType.BATTLEAXE_HACK, FightType.BATTLEAXE_SMASH,
																FightType.BATTLEAXE_BLOCK },
														7499,
														7511), GREATAXE(1698, 1701, 5, new FightType[] {
																FightType.GREATAXE_CHOP, FightType.GREATAXE_HACK,
																FightType.GREATAXE_SMASH, FightType.GREATAXE_BLOCK },
																7499, 7511), CROSSBOW(1764, 1767, 5,
																		new FightType[] { FightType.CROSSBOW_ACCURATE,
																				FightType.CROSSBOW_RAPID,
																				FightType.CROSSBOW_LONGRANGE },
																		7549, 7561), SHORTBOW(1764, 1767, 3,
																				new FightType[] {
	...
	public static void assign(Player player) {
		...
		// player.getPacketSender().sendItemOnInterface(weapon.getInterfaceId()
		// + 1, 200, item);
		// player.getPacketSender().sendItemOnInterface(weapon.getInterfaceId()
		// + 1, item, 0, 1);

		player.getPacketSender().sendTabInterface(0, weapon.getInterfaceId());
		player.getPacketSender().sendString(weapon.getNameLineId(), ItemDefinition.forId(item).getName());
		...
		for (FightType type : weapon.getFightType()) {
			if (type.getStyle() == player.getCombat().getFightType().getStyle()) {
				...
				player.getPacketSender().sendConfig(player.getCombat().getFightType().getParentId(),
						player.getCombat().getFightType().getChildId());
				return;
			}
		}
