...
public class BountyHunter {
		...
		if(player.getLocation() == Location.WILDERNESS) {
			...
			if(target == null) {
				...
				if(stopwatch.elapsed(30000)) {
					...
					//Check the player isn't teleporting or that they have died..
					...
					for(Player other : PLAYERS_IN_WILD) {
						...
						//Check the guy isn't teleporting or that they have died..
						...
						int combatDifference = CombatFactory.combatLevelDifference(player.getSkillManager().getCombatLevel(), other.getSkillManager().getCombatLevel());

						if(combatDifference <= player.getWildernessLevel() && combatDifference <= other.getWildernessLevel()) {
							assign(player, other);
							assign(other, player);
							break;
						}
				...
				if(target.getLocation() == Location.WILDERNESS) {
					...
					if(stopwatch.elapsed(20000)) {
						...
						//If player can't even see the target, let them know where they're at.
						if(!player.getLocalPlayers().contains(target)) {
							player.getPacketSender().sendMessage("Your target is in level "+target.getWildernessLevel()+" wilderness.");
						}
			...
			if(target != null) {
				...
				if(safingTimer == 180 || safingTimer == 120 || safingTimer == 60) {
					player.getPacketSender().sendMessage("You have "+safingTimer+" seconds to get back to the wilderness before you lose your target.");
					target.getPacketSender().sendMessage("Your target has "+safingTimer+" seconds to get back to the wilderness before they lose you as").sendMessage("target.");
	...
	public void updateInterface() {
		...
		.sendString(23324, "Players killed: "+normalKills)
		.sendString(23325, "Deaths: "+deaths);
	}
	...
	public void killedPlayer(Player killed) {
		if(target != null) {
			if(target.equals(killed)) {
				...
				//Of receiving a tier 1 emblem since they successfully killed a target
				if(inventoryEmblem == null) {
					if(true) {
						GroundItemManager.spawnGroundItem(player, new GroundItem(new Item(Emblem.MYSTERIOUS_EMBLEM_1.id, 1), killed.getPosition(), player.getUsername(), player.getHostAddress(), false, 150, false, -1));
						player.getPacketSender().sendMessage("@red@You have been awarded with a mysterious emblem for successfully killing your target.");
					}
				} else {
					...
					if(inventoryEmblem != Emblem.MYSTERIOUS_EMBLEM_10) {
						...
						//This emblem can't be upgraded more..
						player.getPacketSender().sendMessage("@red@Your mysterious emblem is already tier 10 and cannot be upgraded further.");
					}
				}
			} else
		...
		ServerFeed.submit("<img="+ServerFeed.SKULL_SPRITE+">@whi@[@gre@"+Misc.getCurrentServerTime()+"@whi@] @or1@"+player.getUsername()+" @whi@has defeated @yel@"+killed.getUsername()+"@whi@!", 15);
	...
	public static enum Emblem {
		MYSTERIOUS_EMBLEM_1(12746, 1, 5),
		MYSTERIOUS_EMBLEM_2(12748, 1, 10),
		MYSTERIOUS_EMBLEM_3(12749, 1, 15),
		MYSTERIOUS_EMBLEM_4(12750, 1, 20),
		MYSTERIOUS_EMBLEM_5(12751, 1, 25),
		MYSTERIOUS_EMBLEM_6(12752, 1, 30),
		MYSTERIOUS_EMBLEM_7(12753, 1, 35),
		MYSTERIOUS_EMBLEM_8(12754, 1, 40),
		MYSTERIOUS_EMBLEM_9(12755, 1, 45),
		MYSTERIOUS_EMBLEM_10(12756, 1, 50),
