...
public class CombatFactory {
	...
	public static boolean validTarget(Character attacker, Character target) {
		...
		if (target.isNeedsPlacement() || attacker.isNeedsPlacement() || attacker.getPosition().getDistance(target.getPosition()) >= 40) {
			attacker.getCombat().reset();
			return false;
		}
	...
	public static void checkPrayerEffects(Character attacker, Player victim, int damage, CombatType type) {
		...
		if (PrayerHandler.isActivated(victim, PrayerHandler.REDEMPTION) && victim.getHitpoints() <= (victim.getSkillManager().getMaxLevel(Skill.HITPOINTS) / 10)) {
