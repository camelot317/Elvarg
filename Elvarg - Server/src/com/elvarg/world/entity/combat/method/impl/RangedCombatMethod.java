...
public class RangedCombatMethod implements CombatMethod {
	...
	public void onQueueAdd(Character character, Character target) {
		if(character.isPlayer()) {
			...
			new Projectile(character, target, ammo.getProjectileId(), ammo.getProjectileDelay() + 16, ammo.getProjectileSpeed() + 28, ammo.getStartHeight(), ammo.getEndHeight(), 0).sendProjectile();
	...
	public void startAnimation(Character character) {
		...
		if(character.isPlayer()) {
			AmmunitionData ammo = character.getAsPlayer().getCombat().getAmmunition();
			//character.getAsPlayer().performGraphic(new Graphic(ammo.getStartGfxId(), ammo.getStartGfxId() == 2138 ? GraphicHeight.LOW : ammo.getStartHeight() >= 43 ? GraphicHeight.HIGH : GraphicHeight.MIDDLE));
		}
	}
