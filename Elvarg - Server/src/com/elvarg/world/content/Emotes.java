...
import com.elvarg.world.model.Skillcape;
			...
			if (cape != null) {
				...
				if (cape != Skillcape.QUEST_POINT) {
					...
					if (cape.ordinal() < Skill.values().length) {
						...
						Skill skill = Skill.forId(cape.ordinal());
						...
						// Custom capes..
						/*
						 * if(cape == Skillcape.MAX_CAPE) { //Check if all level
						 * 99s for(Skill skill : Skill.values()) { int level =
						 * SkillManager.getMaxAchievingLevel(skill); if
						 * (player.getSkillManager().getMaxLevel(skill) < level)
						 * {
						 * player.getPacketSender().sendMessage("You need "+Misc
						 * .anOrA(skill.getName())+" " +
						 * Misc.formatPlayerName(skill.getName().toLowerCase())
						 * + " level of at least "+ level +
						 * " to do this emote."); return false; } } }
						 */
					}
				}
