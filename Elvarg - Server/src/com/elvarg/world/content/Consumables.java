...
public class Consumables {
	...
	private static void eat(Player player, FoodType food, int slot) {
		 ...
		 * sendMessage("Food has been disabled in this duel."); return; }
		 */
		if (food != null && player.getFoodTimer().elapsed(1100) || food == FoodType.KARAMBWAN) {
			...
			if (heal + player.getSkillManager().getCurrentLevel(Skill.HITPOINTS) > max) {
				...
				// Don't reset hp back to max level.. Player might have used
				// brews
				if (heal < 0) {
					heal = 0;
				}
				...
				5), THIRD_CAKE_SLICE(new Item(1895), 5), BANDAGES(new Item(14640), 12), JANGERBERRIES(new Item(247),
						2), WORM_CRUNCHIES(new Item(2205), 7), EDIBLE_SEAWEED(new Item(403), 4), ANCHOVIES(
								...
								1), SHRIMPS(new Item(315), 3), SARDINE(new Item(325), 4), COD(new Item(339), 7), TROUT(
										new Item(333), 7), PIKE(new Item(351), 8), SALMON(new Item(329), 9), TUNA(
												new Item(361), 10), LOBSTER(new Item(379), 12), BASS(new Item(365),
														13), SWORDFISH(new Item(373), 14), MEAT_PIZZA(new Item(2293),
																14), MONKFISH(new Item(7946), 16), SHARK(new Item(385),
																		20), SEA_TURTLE(new Item(397), 21), MANTA_RAY(
																				new Item(391),
																				22), KARAMBWAN(new Item(3144), 18),
