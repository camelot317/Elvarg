...
public class Shop extends ItemContainer {
	...
	public Shop(Player player, ShopDefinition definition) {
		...
		if(definition.getOriginalStock().length >= capacity()) {
			System.out.println("Too many items in shop id: "+definition.getId()+".  "
					+ "Item amount: "+definition.getOriginalStock().length);
			return;
		}
	...
	public static void checkValue(Player player, int interface_id, int slot_id, int item_id, boolean selling_item) {
		...
		if(player.getShop().getDefinition().getCurrency().getId() != -1) {
			...
			String ending = player.getShop().getDefinition().getCurrency().getDefinition().getName().toLowerCase().endsWith("s") ? player.getShop().getDefinition().getCurrency().getDefinition().getName().toLowerCase() : player.getShop().getDefinition().getCurrency().getDefinition().getName().toLowerCase() + "s";
			...
			message += ""+(int) item_value+" "+ending+""+shopPriceEx((int) item_value)+".";
	...
	public static void buyItem(Player player, int interface_id, int item_id, int slot, int amount) {
		...
		if(player.getShop().getItems()[slot].getAmount() <= 1 && player.getShop().getDefinition().getId() != GENERAL_STORE) {
		...
		if(amount == 0) {
			...
			player.getPacketSender().sendMessage("You can only buy 5000 "+ItemDefinition.forId(item_id).getName()+"s at a time.");
			return;
		}
